<script lang="ts">
  ...
  // 指定日のチェック状況を取得
  function getChecksForDate(date: Date): { hasMorning: boolean; hasNight: boolean } {
...
<div class="absolute top-8 left-3 sm:top-8 sm:left-5">
  <a href="/" class="px-3 sm:px-4 py-2 text-sm sm:text-base rounded-md bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 transition-colors">
    ← 今日
  </a>
</div>
  <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold mb-6 sm:mb-8">週間チェック状況</h1>
  ...
  {:else}
      ...
      {#each weekData as day (day.date.toISOString())}
        ...
        >
          <div class="flex items-center justify-between">
            <div class="text-left">
              <p class="font-semibold text-sm sm:text-base">{formatDate(day.date)}</p>
              {#if isToday(day.date)}
          ...
          <div class="text-xs text-neutral-500 dark:text-neutral-400 mt-2 text-left">
            <p>朝: {day.hasMorning ? "6:00～12:00 完了" : "未実施"}</p>
            <p>夜: {day.hasNight ? "18:00～25:00 完了" : "未実施"}</p>
          </div>
        </div>
      {/each}
    </div>
    ...
    {#if weekData.length > 0}
      <div class="mt-8 sm:mt-10 p-4 sm:p-6 rounded-lg bg-neutral-100 dark:bg-neutral-800 w-full max-w-2xl">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
          <div class="text-center">
            ...
            <p class="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400">朝チェック</p>
          </div>
          <div class="text-center">
            <p class="text-2xl sm:text-3xl font-bold text-green-600 dark:text-green-400">
              {weekData.filter((d) => d.hasNight).length}
            </p>
            <p class="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400">夜チェック</p>
          </div>
          <div class="text-center">
            <p class="text-2xl sm:text-3xl font-bold text-blue-600 dark:text-blue-400">
              {weekData.filter((d) => d.hasMorning && d.hasNight).length}
            </p>
            <p class="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400">完全達成</p>
          </div>
        </div>
      </div>
    {/if}
